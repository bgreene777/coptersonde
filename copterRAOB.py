...
if autofile == 'y':
    filename = wxtoolsPC.findLatestCSV(dataDirName)
elif autofile == 'n':
    ...
    filename = ''
    for i in range(len(ftemp) - 1):
        filename = filename + ftemp[i] + '\\'
    filename += ftemp[-1]
else:
...
if dgpsFlag == 'y':
    dgpsFlag = 1
    ppkname = wxtoolsPC.findDGPSfile(filename)
...
if dgpsFlag:
    dgpsdata = wxtoolsPC.dgpsread(ppkname)
...
copterdata = wxtoolsPC.csvread_raw(filename)
time = copterdata[0]
lat = copterdata[1]
lon = copterdata[2]
alt = copterdata[3]
...
if isfindsite == 'y':
    meso = wxtoolsPC.findSite(lat[0], lon[0])
...
if profup:
    timeTakeoff = wxtoolsPC.findStart(vspd, alt, t_copter)
    if dgpsFlag:
...
if profdown:
    ...
    if dgpsFlag:
        ...
        dt_copter = [time + timeDiff for time in dt_copter]
        t_copter = mpdates.date2num(dt_copter)
        timeEnd = wxtoolsPC.findEnd(vspd, alt, t_copter)
...
if isRH:
    ...
    # CAPE
    SBCAPE = wxtoolsPC.uavCAPE(Tmean * units.degC, prof, pres * units.hPa)
...
## Import Mesonet Data ##
#########################
mesoData = wxtoolsPC.getMesoData(timeTakeoff.year, timeTakeoff.month,
...
if mesoData.size == 0:
    ...
    iMesoTime = wxtoolsPC.findClosestMesoTime(timeTakeoff)
...
skew.plot(pmeso, T2meso, 'r*', linewidth=2, label='Mesonet 2m T')
skew.plot(pmeso, Td2meso, 'g*', linewidth=2, label='Mesonet 2m Td')
skew.plot(pres[0], T9meso, 'k*', linewidth=2, label='Mesonet 9m T')
...
while s != 'y' and s != 'n':
    s = raw_input('>>>Save csv and figures? y/n ')
if s == 'y':
    saveFileName = folderSaveFile + '%s-OUUAS%s-%s.csv' % \
    ...
    # png
    saveFileNamePNG = folderSavePNG + '%s-OUUAS%s-%s.png' % \
        (timeTakeoff.strftime('%Y%m%d_%H%M%S'), copterNum, sitename)
